    ...
    with open(filepath, "r") as file:
        ...
        for row in reader:
            ...
            if previous_time is not None and (
                time_in_seconds - previous_time > time_gap_threshold
            ):
...
# Read data files
tip_pos_start_time = read_csv_to_dict(
    tip_pos_file,
    tip_pos_data,
...
fig, ax = plt.subplots(
    7, 1, gridspec_kw={"height_ratios": ratios}, figsize=(5.5 * 1, 2 * len(ratios))
)
...
ax[4].plot(
    set_ik_data["time"],
    set_ik_data["qx"],
    label="Set IK Target QX",
    color=ieee_blue,
    # linewidth=1.5,
)
# ax[4].spines["bottom"].set_visible(False)
ax[4].set_ylabel("QX")
clean_axes(ax[4], tip_pos_data["time"])
...
ax[5].plot(
    set_ik_data["time"],
    set_ik_data["qy"],
    label="Set IK Target QY",
    color=ieee_blue,
    # linewidth=1.5,
)
ax[5].set_ylabel("QY")
clean_axes(ax[5], tip_pos_data["time"])
...
ax[6].plot(
    set_ik_data["time"],
    set_ik_data["qz"],
    label="Set IK Target QZ",
    color=ieee_blue,
    # linewidth=1.5,
)
ax[6].set_xlabel("Time (s)")
ax[6].set_ylabel("QZ")
clean_axes(ax[6], tip_pos_data["time"])
...
plt.tight_layout()
plt.savefig("full.pdf", format="pdf")
plt.close()

# # QX
# ax[0].plot(
#     tip_pos_data["time"], tip_pos_data["qx"], label="Actual Tip QX", color=ieee_black
# )
# ax[0].plot(
#     set_ik_data["time"],
#     set_ik_data["qx"],
...
# # QY
# ax[1].plot(
#     tip_pos_data["time"], tip_pos_data["qy"], label="Actual Tip QY", color=ieee_black
# )
# ax[1].plot(
#     set_ik_data["time"],
#     set_ik_data["qy"],
...
# # QZ
# ax[2].plot(
#     tip_pos_data["time"], tip_pos_data["qz"], label="Actual Tip QZ", color=ieee_black
# )
# ax[2].plot(
#     set_ik_data["time"],
#     set_ik_data["qz"],
...
# # QW
# ax[3].plot(
#     tip_pos_data["time"], tip_pos_data["qw"], label="Actual Tip QW", color=ieee_black
# )
# ax[3].plot(
#     set_ik_data["time"],
#     set_ik_data["qw"],
