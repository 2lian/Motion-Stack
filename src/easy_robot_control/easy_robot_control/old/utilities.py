...
pi_over_3_zquat = qt.from_rotation_vector(
    np.array([0, 0, np.pi / 3], dtype="float32")
)
# precomputed quaternion for pi/3 rotation
float32 = np.float32

leg_dim = {
    "body": 181,  # dimensions of the robot
    "coxa": 64,
    "femur": 129,
    "tibia": 160,
    "a_coxa": 60,
...
default_leg0_tip = np.array(
    [461, 0, 0], dtype="float32"
)  # coord of the tip of leg0
...
ROTATION_MATRIX_LIST = np.array(
    [quaternion.as_rotation_matrix(pi_over_3_zquat ** (-1 * i)) for i in range(6)]
).astype("float32")


@njit("complex64[:](float32[:,::2])", cache=USE_CACHE)
...
def rotate_points(points: np.ndarray, z_angles: np.ndarray) -> np.ndarray:
    ...
    points_xy_rotated = complexify(rotated_points[:, :2]) * np.exp(z_angles * j)
    rotated_points[:, 0], rotated_points[:, 1] = np.real(
        points_xy_rotated
    ), np.imag(points_xy_rotated)
    return rotated_points


@njit(["float32[:,::6,::3](float32[:,::6,::3])"], cache=USE_CACHE)
...
@njit(["float32[:,::3](float32[:,::6,::3])"], cache=USE_CACHE)
def average_of_holdset(holdset_array: np.ndarray) -> np.ndarray:
    ...
    count_of_non_nan_values = np.sum(
        ~np.isnan(holdset_array[:, :, 0]), axis=1
    ).reshape((hold_3D_nan_as_0.shape[0], 1))
...
def avg2zero_holdset(holdset_array: np.ndarray) -> np.ndarray:
    ...
    return holdset_array - np.reshape(
        np.ascontiguousarray(average_of_holdset(holdset_array)),
...
@njit("Tuple((float32[:,::3],float32[:]))(float32[::6,::3])", cache=USE_CACHE)
def holdset_to_point_angle_numba(
    holdset: np.ndarray,
...
# @njit("float32[:,::6,::3](float32[:,::6,::3], int8)", cache=USE_CACHE)
# @numba.njit()
def rotate_by_pithree(holdset_array: np.ndarray, nb_of_pi3: int) -> np.ndarray:
    ...
    ev3col_rotated = everthing_in_3_columns @ ROTATION_MATRIX_LIST[nb_of_pi3 % 6]
...
@njit("boolean[:](float32[:,::6,::3], float32)", cache=USE_CACHE)
def dist2adjacent_is_enough(
    holdset_array: np.ndarray, min_dist_allowed: float
) -> np.ndarray:
    """
    ...
    differences = (
        holdset_array[
            :, np.arange(1, holdset_array.shape[1] + 1) % holdset_array.shape[1], :
        ]
        - holdset_array
    )
...
# @njit("boolean[:](float32[:,::6,::3])", cache=USE_CACHE)
