...
from numba import njit, prange, bool_
...
min_tibia_to_gripper_dist = np.abs(
    tibia_length + femur_length * np.exp(np.deg2rad(femur_angle_deg) * 1j)
)
...
@njit("float32[::3](float32[::3])", fastmath=FAST_MATH, cache=USE_CACHE)
def vect_to_avg_surf(point: np.ndarray) -> np.ndarray:
    ...
    p_in_horizontal_complex_plane = (
        point_relative_to_coxa_origin[0] + point_relative_to_coxa_origin[1] * 1j
    )
    ...
    horizontal_complex_plane_with_x_following_coxa = (
        p_in_horizontal_complex_plane * np.exp(-real_coxa_angle * 1j)
    )

    solution[1] = np.imag(horizontal_complex_plane_with_x_following_coxa)

    vertical_complex_tibia_plane = (
        np.real(horizontal_complex_plane_with_x_following_coxa - coxa_length)
        + point[2] * 1j
    )
    ...
    tibia_plane_with_origin_on_the_avg_line = (
        vertical_complex_tibia_plane - middle_TG * np.exp(real_tibia_angle * 1j)
    )

    solution[0] = np.real(tibia_plane_with_origin_on_the_avg_line)
    solution[2] = np.imag(tibia_plane_with_origin_on_the_avg_line)
    align_solution_back_to_xy_plane = (solution[0] + solution[1] * 1j) * np.exp(
        real_coxa_angle * 1j
    )
...
@njit("float32(float32[::3])", fastmath=FAST_MATH, cache=USE_CACHE)
...
@njit("boolean(float32[::3])", fastmath=FAST_MATH, cache=USE_CACHE)
def is_reachable(point: np.ndarray) -> bool:
    ...
    p_in_horizontal_complex_plane = (
        point_with_coxa_as_origin[0] + point_with_coxa_as_origin[1] * 1j
    )
    ...
    tibia_plane_with_origin_on_femur = (
        vertical_complex_tibia_plane - tibia_length * np.exp(real_tibia_angle * 1j)
    )
...
@njit(
    "float32[:,::3](float32[:,::3], float32)", fastmath=FAST_MATH, cache=USE_CACHE
)
def dist_to_avg_surf_PAarray(points, single_angle):
    unrotated = ut.rotate_points(points, -single_angle)
    distance = np.empty_like(points)
    for row in prange(points.shape[0]):
        distance[row, :] = dist_to_avg_surf(unrotated[row, :])
    rotated_back = ut.rotate_points(distance, single_angle)
    return rotated_back


@njit("boolean[:](float32[:,::3], float32)", cache=USE_CACHE)
def is_reachable_array(points, single_angle):
    unrotated = ut.rotate_points(points, -single_angle)
    reachability = np.empty((points.shape[0],), bool_)
    for row in prange(points.shape[0]):
        reachability[row] = is_reachable_no_under_coxa(unrotated[row, :])
    return reachability
