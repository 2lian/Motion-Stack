...
def shift_array(array: np.ndarray, direction: np.int32) -> np.ndarray:
    ...
    array_shifted = array_shifted[(np.arange(
        array_shifted.shape[0], dtype=np.int32) + direction) % len(array_shifted)]
...
def remove_concave_points(
    ...
    angle_index = (angles <= np.pi)
    points = holdset[angle_index]
    return points


@numba.njit("complex64[:](complex64[:], complex64)")
def vectors_point_to_holdset(
        holdset: np.ndarray,
...
def stability_arrays(holdset: np.ndarray,
                     point: np.ndarray) -> Tuple[np.ndarray,
                                            np.ndarray]:
    # Remove nan values
...
def is_point_stable(holdset: np.ndarray, point: np.ndarray) -> bool:
    ...
    vectors_to_convex_vertex1 = vectors_point_to_holdset(
        holdset_stab, point_stab)
    vectors_to_convex_vertex2 = shift_array(vectors_to_convex_vertex1, 1)

    cross_products = np.imag(
        np.conj(vectors_to_convex_vertex1) *
        vectors_to_convex_vertex2)
...
def stability_vector(
    ...
    proj_ab = a.real + (dot_abap / (ab.real**2 + ab.imag**2)) * ab.real + \
    ...
    if inside:
        ...
        all_distances = np.append(
            np.sqrt(
                new_vector_ap_proj.real**2 +
                new_vector_ap_proj.imag**2),
            np.abs(pa))
    ...
    ssm_vector_float32 = np.concatenate(
        (ssm_vector.real.astype(
            np.float32), ssm_vector.imag.astype(
            np.float32)))

    return np.float32(1.01) * ssm_vector_float32
