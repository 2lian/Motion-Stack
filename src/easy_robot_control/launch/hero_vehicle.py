...
xacro_path = xacro_path[: -len(ROBOT_NAME + ".xacro")] + "hero_vehicle.xacro"
...
for leg_index, ee_name in zip(leg_indices, LEG_END_EFF):
    ...
    if is_wheel(leg_index):
        ...
        this_node_param["start_effector_name"] = this_node_param["end_effector_name"]
        this_node_param["speed_mode"] = True
        this_node_param["leg_list"] = [0]

    else:
        this_node_param["add_joints"] = [f"leg{leg_index}grip1", f"leg{leg_index}grip2"]
    if leg_index == leg_indices[0]:
        # this_node_param["add_joints"] = [f"{wind}wheel_{lr}_joint" for wind in [1,2,3,4] for lr in ["right", "left"]]
        pass
    else:
