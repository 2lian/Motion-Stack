...
int Arm_Socket_Start_Connect(void) {
  ...
  // 查看连接状态
  Arm_connect =
      connect(Arm_Socket, (struct sockaddr *)&serAddr, sizeof(serAddr));
  // ROS_INFO("Arm_connect=%d\n",Arm_connect);
  if (Arm_connect != 0) {
    if (errno != EINPROGRESS) // connect返回错误。
    {
      std::cout << "Arm_connect=" << Arm_connect << "connect failed"
                << std::endl;
      close(Arm_Socket);
      return 3;
    } else {
      ...
      if (res <= 0) {
        std::cout << "********************Connection failed, check your "
                     "connection!**************"
                  << std::endl;
        close(Arm_Socket);
        return 3;
      ...
      if (FD_ISSET(Arm_Socket, &wset)) {
        ...
        if (getsockopt(Arm_Socket, SOL_SOCKET, SO_ERROR, &err, &len) <
            0) // 两种错误处理方式
        {
...
void Arm_Close(void) { Rm_Api.Service_Arm_Socket_Close(m_sockhand); }
...
void RmArm::Arm_MoveJ_Callback(rm_ros_interfaces::msg::Movej::SharedPtr msg) {
  ...
  block = msg->block;
  res = Rm_Api.Service_Movej_Cmd(m_sockhand, joint, v, 0, trajectory_connect,
                                 block);
  movej_data.data = res;
...
void RmArm::Arm_MoveL_Callback(rm_ros_interfaces::msg::Movel::SharedPtr msg) {
  ...
  block = msg->block;
  trajectory_connect = msg->trajectory_connect;
  res = Rm_Api.Service_Movel_Cmd(m_sockhand, pose, v, 0, trajectory_connect,
                                 block);
  movel_data.data = res;
...
void RmArm::Arm_MoveC_Callback(rm_ros_interfaces::msg::Movec::SharedPtr msg) {
  ...
  block = msg->block;
  trajectory_connect = msg->trajectory_connect;
  res = Rm_Api.Service_Movec_Cmd(m_sockhand, pose_via, pose_to, v, 0, loop,
...
void RmArm::Arm_Movej_CANFD_Callback(
    rm_ros_interfaces::msg::Jointpos::SharedPtr msg) {
  float joint[7];
  bool follow;
  float expand;
  u_int32_t res;
...
void RmArm::Arm_Movep_CANFD_Callback(
    rm_ros_interfaces::msg::Cartepos::SharedPtr msg) {
...
void RmArm::Arm_MoveJ_P_Callback(
    rm_ros_interfaces::msg::Movejp::SharedPtr msg) {
  ...
  trajectory_connect = msg->trajectory_connect;
  block = msg->block;
  res = Rm_Api.Service_Movej_P_Cmd(m_sockhand, pose, v, 0, trajectory_connect,
                                   block);
...
void RmArm::Set_Joint_Teach_Callback(
    rm_ros_interfaces::msg::Jointteach::SharedPtr msg) {
  byte num;
  byte direction;
  byte v;
  bool block;
...
void RmArm::Set_Pos_Teach_Callback(
    rm_ros_interfaces::msg::Posteach::SharedPtr msg) {
  byte type;
  byte direction;
  byte v;
  bool block;
  ...
  res = Rm_Api.Service_Pos_Teach_Cmd(m_sockhand, (POS_TEACH_MODES)type,
                                     direction, v, block);
...
void RmArm::Set_Ort_Teach_Callback(
    rm_ros_interfaces::msg::Ortteach::SharedPtr msg) {
  byte type;
  byte direction;
  byte v;
  bool block;
  ...
  res = Rm_Api.Service_Ort_Teach_Cmd(m_sockhand, (ORT_TEACH_MODES)type,
                                     direction, v, block);
...
void RmArm::Arm_Get_Realtime_Push_Callback(
  ...
  copy = msg;
  res = Rm_Api.Service_Get_Realtime_Push(m_sockhand, &cycle, &port, &enable,
                                         &force_coordinate, ip);
  if (res == 0) {
...
void RmArm::Arm_Set_Realtime_Push_Callback(
  ...
  res = Rm_Api.Service_Set_Realtime_Push(m_sockhand, cycle, port, true,
                                         force_coordinate, ip);
  if (res == 0) {
...
void RmArm::Set_UDP_Configuration(int udp_cycle, int udp_port,
                                  int udp_force_coordinate,
                                  std::string udp_ip) {
  int cycle;
  int port;
  int force_coordinate;
  ...
  if (res == 0) {
    RCLCPP_INFO(
        this->get_logger(),
        "UDP_Configuration is cycle:%dms,port:%d,force_coordinate:%d,ip:%s\n",
...
void RmArm::Get_Arm_Version() {
  ...
  char version;
  u_int32_t res;
  res = Rm_Api.Service_Get_Arm_Software_Version(m_sockhand, plan_version,
                                                ctrl_version, kernal1, kernal2,
                                                product_version);
  if (res == 0) {
    ...
    if (version == 'B') {
      ...
      // RCLCPP_INFO (this->get_logger(),"control_version =
      // %d",Udp_RM_Joint.control_version);
    }
    if (version == 'F') {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Service_Get_Arm_Software_Version error = %d", res);
  }
}

void RmArm::Arm_Get_Arm_Software_Version_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  char plan_version[50];
  char ctrl_version[50];
  char kernal1[50];
...
void RmArm::Arm_Start_Force_Position_Move_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
...
void RmArm::Arm_Stop_Force_Position_Move_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  u_int32_t res;
...
void RmArm::Arm_Force_Position_Move_Joint_Callback(
  ...
  force = msg->force;
  follow = msg->follow;
  res = Rm_Api.Service_Force_Position_Move_Joint(m_sockhand, joint, sensor,
                                                 mode, dir, force, follow);
  if (res != 0) {
    RCLCPP_INFO(this->get_logger(),
                "Arm force position move joint error code is %d\n", res);
  }
}
...
//     follow = msg->follow;
//     res = Rm_Api.Service_Force_Position_Move_Joint(m_sockhand, joint, sensor,
//     mode, dir, force, follow); if(res != 0)
//     {
//         RCLCPP_INFO (this->get_logger(),"Arm force position move joint error
//         code is %d\n",res);
//     }
// }
...
void RmArm::Arm_Force_Position_Move_Pose_Callback(
  ...
  force = msg->force;
  follow = msg->follow;
  res = Rm_Api.Service_Force_Position_Move_Pose(m_sockhand, joint_pose, sensor,
                                                mode, dir, force, follow);
  if (res != 0) {
    RCLCPP_INFO(this->get_logger(),
                "Arm force position move pose error code is %d\n", res);
  }
}
...
void RmArm::Arm_Set_Force_Postion_Callback(
  ...
  N = msg->n;
  block = msg->block;
  res = Rm_Api.Service_Set_Force_Postion(m_sockhand, sensor, mode, direction, N,
                                         block);
  if (res == 0) {
    ...
    this->Set_Force_Postion_Result->publish(arm_set_force_postion_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm_set_force_postion_callback error code is %d\n", res);
  }
}
void RmArm::Arm_Stop_Force_Postion_Callback(
...
void RmArm::Arm_Change_Work_Frame_Callback(
  ...
  if (res == 0) {
    ...
    this->Change_Work_Frame_Result->publish(arm_change_work_frame_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm_change_work_frame_callback error code is %d\n", res);
  }
}

void RmArm::Arm_Get_Curr_WorkFrame_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  FRAME frame;
  u_int32_t res;
  ...
  if (res == 0) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arm_get_curr_workFrame_callback error code is %d\n", res);
  }
}

void RmArm::Arm_Get_Current_Tool_Frame_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  FRAME frame;
  u_int32_t res;
  std_msgs::msg::String curr_frame;
...
void RmArm::Arm_Get_All_Tool_Frame_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  FRAME_NAME name[10];
  u_int32_t res;
  ...
  if (res == 0 && len <= 10) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arm_get_all_tool_frame_callback error code is %d\n", res);
  }
}

void RmArm::Arm_Get_All_Work_Frame_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  FRAME_NAME name[10];
  u_int32_t res;
  ...
  if (res == 0 && len <= 10) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arm_get_all_work_frame_callback error code is %d\n", res);
  }
}

void RmArm::Arm_Set_Tool_Voltage_Callback(
...
void RmArm::Arm_Set_Joint_Err_Clear_Callback(
  ...
  if (res == 0) {
    ...
    this->Set_Joint_Err_Clear_Result->publish(set_joint_err_clear_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm set joint err clear callback error code is %d\n", res);
  }
}
...
void RmArm::Arm_Set_Gripper_Pick_On_Callback(
  ...
  if (res == 0) {
    ...
    this->Set_Gripper_Pick_On_Result->publish(set_gripper_pick_on_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm set gripper pick on error code is %d\n", res);
  }
}
...
void RmArm::Arm_Set_Gripper_Pick_Callback(
  ...
  timeout = msg->timeout;
  res =
      Rm_Api.Service_Set_Gripper_Pick(m_sockhand, speed, force, block, timeout);
...
void RmArm::Arm_Set_Gripper_Position_Callback(
  ...
  timeout = msg->timeout;
  res =
      Rm_Api.Service_Set_Gripper_Position(m_sockhand, position, block, timeout);
  ...
  if (res == 0) {
    ...
    this->Set_Gripper_Position_Result->publish(set_gripper_position_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm set gripper position error code is %d\n", res);
  }
}
...
void RmArm::Arm_Set_Hand_Seq_Callback(
    const rm_ros_interfaces::msg::Handseq::SharedPtr msg) {
  int seq_num;
  bool block;
  u_int32_t res;
...
void RmArm::Arm_Set_Hand_Angle_Callback(
...
void RmArm::Arm_Set_Hand_Speed_Callback(
    const rm_ros_interfaces::msg::Handspeed::SharedPtr msg) {
  int speed;
  bool block;
  u_int32_t res;
...
void RmArm::Arm_Set_Hand_Force_Callback(
    const rm_ros_interfaces::msg::Handforce::SharedPtr msg) {
  int force;
  bool block;
  u_int32_t res;
...
void RmArm::Arm_Set_Lift_Speed_Callback(
    const rm_ros_interfaces::msg::Liftspeed::SharedPtr msg) {
  int speed;
  u_int32_t res;
  ...
  if (res == 0) {
    ...
    this->Set_Lift_Speed_Result->publish(set_lift_speed_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm set lift speed result error code is %d\n", res);
  }
}

void RmArm::Arm_Set_Lift_Height_Callback(
    const rm_ros_interfaces::msg::Liftheight::SharedPtr msg) {
  int speed;
  int height;
  bool block;
  u_int32_t res;
  ...
  if (res == 0) {
    ...
    this->Set_Lift_Height_Result->publish(set_lift_height_result);
    RCLCPP_INFO(this->get_logger(),
                "Arm set lift height result error code is %d\n", res);
  }
}

void RmArm::Arm_Get_Lift_State_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  int current;
  int height;
  int err_flag;
  ...
  if (res == 0) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arm set lift state result error code is %d\n", res);
  }
}

void RmArm::Arm_Get_Current_Arm_State_Callback(
    const std_msgs::msg::Empty::SharedPtr msg) {
  Pose pose;
  float joint[7];
  u_int16_t Arm_Err;
  ...
  Quat quat;
  int i;
  res = Rm_Api.Service_Get_Current_Arm_State(m_sockhand, joint, &pose, &Arm_Err,
                                             &Sys_Err);
  if (res == 0) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arm get current arm state error code is %d\n", res);
  }
}

void RmArm::Arm_Clear_Force_Data_Callback(
    const std_msgs::msg::Bool::SharedPtr msg) {
  bool block;
  u_int32_t res;
...
bool UdpPublisherNode::read_data() {
  ...
  ssize_t numBytes =
      recvfrom(16, udp_socket_buffer, sizeof(udp_socket_buffer), 0,
  ...
  std::cout << "Received from "       //<< inet_ntoa(clientAddr.sin_addr)
            << ":" << ntohs(clientAddr.sin_port) << ": " << udp_socket_buffer
            << std::endl;
  if ((udp_socket_buffer[numBytes - 2] == 0X0D) &&
      (udp_socket_buffer[numBytes - 1] == 0X0A)) {
    return true;
  } else {
...
UdpPublisherNode::UdpPublisherNode() : rclcpp::Node("udp_publish_node") {
  ...
  Six_Zero_Force_Result =
      this->create_publisher<rm_ros_interfaces::msg::Sixforce>(
  ...
  Joint_Error_Code_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointerrorcode>(
          "rm_driver/udp_joint_error_code", 10); // 发布当前的关节错误码
  Sys_Err_Result = this->create_publisher<std_msgs::msg::UInt16>(
      "rm_driver/udp_sys_err", 10); // 发布当前的系统错误码
  Arm_Err_Result = this->create_publisher<std_msgs::msg::UInt16>(
      "rm_driver/udp_arm_err", 10); // 发布当前的机械臂错误码
...
RmArm::RmArm() : rclcpp::Node("rm_driver") {
  ...
  Get_Realtime_Push_Result =
      this->create_publisher<rm_ros_interfaces::msg::Setrealtimepush>(
  ...
  Get_Realtime_Push_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Realtime_Push_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /******************************************************设置udp配置********************************************************************/
  ...
  Set_Realtime_Push_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Setrealtimepush>(
          "rm_driver/set_realtime_push_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Realtime_Push_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /******************************************************************************end*******************************************************************/
  ...
  MoveC_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/movec_result", rclcpp::ParametersQoS());
  ...
  Move_Stop_Cmd = this->create_subscription<std_msgs::msg::Bool>(
  ...
  Set_Joint_Teach_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Jointteach>(
          "rm_driver/set_joint_teach_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Set_Joint_Teach_Callback, this,
                    std::placeholders::_1),
          sub_opt4);
  /*********************************************************位置示教*****************************************************************/
  ...
  Set_Pos_Teach_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Posteach>(
          "rm_driver/set_pos_teach_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Set_Pos_Teach_Callback, this,
                    std::placeholders::_1),
          sub_opt4);
  /*********************************************************姿态示教*****************************************************************/
  ...
  Get_Arm_Software_Version_Result =
      this->create_publisher<rm_ros_interfaces::msg::Armsoftversion>(
  ...
  Get_Arm_Software_Version_Cmd =
      this->create_subscription<std_msgs::msg::Empty>(
          ...
          std::bind(&RmArm::Arm_Get_Arm_Software_Version_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /*********************************************************************************end*******************************************************************/
  ...
  Start_Force_Position_Move_Cmd =
      this->create_subscription<std_msgs::msg::Empty>(
          "rm_driver/start_force_position_move_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Start_Force_Position_Move_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /********************************************************关闭力位混合*******************************************************************/
  ...
  Stop_Force_Position_Move_Cmd =
      this->create_subscription<std_msgs::msg::Empty>(
          "rm_driver/stop_force_position_move_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Stop_Force_Position_Move_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /********************************************************角度透传力位混合*****************************************************************/
  Force_Position_Move_Joint_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Forcepositionmovejoint>(
          ...
          std::bind(&RmArm::Arm_Force_Position_Move_Joint_Callback, this,
                    std::placeholders::_1),
          sub_opt4);
  /********************************************************位姿透传力位混合*****************************************************************/
  Force_Position_Move_Pose_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Forcepositionmovepose>(
          ...
          std::bind(&RmArm::Arm_Force_Position_Move_Pose_Callback, this,
                    std::placeholders::_1),
          sub_opt4);
  /********************************************************设置力位混合控制*******************************************************************/
  Set_Force_Postion_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/set_force_postion_result", rclcpp::ParametersQoS());
  Set_Force_Postion_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Setforceposition>(
          "rm_driver/set_force_postion_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Force_Postion_Callback, this,
                    std::placeholders::_1),
          sub_opt4);
  /********************************************************结束力位混合控制*******************************************************************/
  ...
  Stop_Force_Postion_Cmd = this->create_subscription<std_msgs::msg::Bool>(
      "rm_driver/stop_force_postion_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_Stop_Force_Postion_Callback, this,
                std::placeholders::_1),
      sub_opt4);
  /****************************************************************************end******************************************************************/
  ...
  Change_Work_Frame_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Arm_Change_Work_Frame_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /**********************************************************获得工作坐标系********************************************************************/
  ...
  Get_Curr_WorkFrame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Curr_WorkFrame_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /**********************************************************获得工具坐标系********************************************************************/
  ...
  Get_Current_Tool_Frame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Current_Tool_Frame_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /**********************************************************获得所有工具坐标系********************************************************************/
  Get_All_Tool_Frame_Result =
      this->create_publisher<rm_ros_interfaces::msg::Getallframe>(
  ...
  Get_All_Tool_Frame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_All_Tool_Frame_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /**********************************************************获得所有工作坐标系********************************************************************/
  ...
  Get_All_Work_Frame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_All_Work_Frame_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Tool_Voltage_Cmd = this->create_subscription<std_msgs::msg::UInt16>(
      ...
      std::bind(&RmArm::Arm_Set_Tool_Voltage_Callback, this,
                std::placeholders::_1),
      sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Joint_Err_Clear_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Jointerrclear>(
          "rm_driver/set_joint_err_clear_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Joint_Err_Clear_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Gripper_Position_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Gripperset>(
          "rm_driver/set_gripper_position_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Gripper_Position_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Set_Hand_Posture_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Handposture>(
          "rm_driver/set_hand_posture_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Hand_Posture_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /***************************************设置灵巧手动作序列序号*********************************/
  Set_Hand_Seq_Result = this->create_publisher<std_msgs::msg::Bool>(
      "rm_driver/set_hand_seq_result", rclcpp::ParametersQoS());
  ...
  Set_Hand_Angle_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Handangle>(
          "rm_driver/set_hand_angle_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Hand_Angle_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /*******************************************设置灵巧手速度************************************/
  ...
  Set_Hand_Speed_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Handspeed>(
          "rm_driver/set_hand_speed_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Hand_Speed_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /*******************************************设置灵巧手角度************************************/
  ...
  Set_Hand_Force_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Handforce>(
          "rm_driver/set_hand_force_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Hand_Force_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Set_Lift_Speed_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Liftspeed>(
          "rm_driver/set_lift_speed_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Lift_Speed_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /****************************************设置升降机构高度**********************************/
  ...
  Set_Lift_Height_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Liftheight>(
          "rm_driver/set_lift_height_cmd", rclcpp::ParametersQoS(),
          std::bind(&RmArm::Arm_Set_Lift_Height_Callback, this,
                    std::placeholders::_1),
          sub_opt3);
  /****************************************获取升降机构状态**********************************/
  Get_Lift_State_Result =
      this->create_publisher<rm_ros_interfaces::msg::Liftstate>(
  ...
  Get_Lift_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Lift_State_Callback, this,
                std::placeholders::_1),
      sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Get_Current_Arm_State_Result =
      this->create_publisher<rm_ros_interfaces::msg::Armstate>(
